...
function show_unlock_location(){
	...
	if(to_the_tenth(1000, count_object(gamedata['known_locations'])) > gamedata['coins_gained'] && count_object(gamedata['known_locations']) != 0)
	{
...
function unlock_location(location_id, free_location){
	...
	if(gamedata['known_locations'][location_id] != undefined || (to_the_tenth(1000, count_object(gamedata['known_locations'])) > gamedata['coins_gained'] && count_object(gamedata['known_locations']) != 0))
	{
		can_find = false;
	}
	else
...
function show_known_locations_list(){
	...
	$.each(gamedata['known_locations'], function(location_id, useless_data){
		...
		parsed_know_location += 		'<div class="known_location" style="background-image:url(images/' + available_locations[location_id]['image'] + ')" onclick="current_location=\'' + location_id + '\';show_content(\'current_location\')">';
	...
	if((to_the_tenth(1000, count_object(gamedata['known_locations'])) <= gamedata['coins_gained'] && count_object(gamedata['known_locations']) < count_object(available_locations)) || count_object(gamedata['known_locations']) == 0)
	{
		var parsed_know_location = '';
	...
	else
	{
		...
		{
			var next_location_value = nFormatter(to_the_tenth(1000, count_object(gamedata['known_locations'])));
			var parsed_know_location = '';
